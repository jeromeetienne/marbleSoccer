...
{
	var position	= this._mesh.position.clone();
	var map		= world.map();
	
// TODO refactor this and the function in map
	for(var deltaY = 0; deltaY > -15*Marble.tileSize; deltaY -= Marble.tileSize){
		var voxelPresent	= map.voxelPresent(position.x, position.y + deltaY, position.z);
		if( voxelPresent === false )	continue;
	
		this._shadowMesh.position.y	= deltaY+0.1;	// +0.001 to ensure the map is above the ground
// TODO this is buggy because the deltaY is always a multiple of Marble.tileSize
