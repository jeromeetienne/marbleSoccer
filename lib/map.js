...
{
	if( false ){
		var mapMinX	= +Number.MAX_VALUE;
		var mapMaxX	= -Number.MAX_VALUE;
		var mapMinY	= +Number.MAX_VALUE;
		var mapMaxY	= -Number.MAX_VALUE;
		var mapMinZ	= +Number.MAX_VALUE;
		var mapMaxZ	= -Number.MAX_VALUE;
		this._mapVoxels.forEach(function(mapVoxel){
			if( mapMinX > mapVoxel.x )	mapMinX	= mapVoxel.x;	
			if( mapMaxX < mapVoxel.x )	mapMaxX	= mapVoxel.x;	
			if( mapMinY > mapVoxel.y )	mapMinY	= mapVoxel.y;
			if( mapMaxY < mapVoxel.y )	mapMaxY	= mapVoxel.y;	
			if( mapMinZ > mapVoxel.z )	mapMinZ	= mapVoxel.z;	
			if( mapMaxZ < mapVoxel.z )	mapMaxZ	= mapVoxel.z;	
		})		
	}
	...
	if( false ){
		...
		var mapW	= 17;
		var mapD	= 17;
		var geometry	= new THREE.CubeGeometry( mapW*Marble.tileSize, Marble.tileSize, mapD*Marble.tileSize, mapW, 1, mapD);
...
// NOTE: it may be faster if aggregated by voxel
