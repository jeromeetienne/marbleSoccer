...
Marble.Marble.prototype.tick	= function()
{
	...
	var mesh	= this._ballMesh;
        ...
        var M = new THREE.Matrix4().setTranslation(0,this._radius,0);
        M.multiplySelf(new THREE.Matrix4().setRotationZ(rotationZ));
        M.multiplySelf(new THREE.Matrix4().setRotationX(rotationX));
        M.multiplySelf(new THREE.Matrix4().setTranslation(0,-this._radius,0));
        mesh.matrix.copy(M.multiplySelf(mesh.matrix));
	//
